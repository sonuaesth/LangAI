...
export function SentencesView() {
  ...
  async function add() {
    ...
    try { await api.addSentences(texts, targetLanguage, translationComment, topic); setText(""); await load(); }
  ...
  return <>
    ...
    <div className="list">{rows.length ? rows.map(sentence => <article className="sentence" key={sentence.id}><input type="checkbox" checked={selected.has(sentence.id)} onChange={() => setSelected(current => { const next = new Set(current); next.has(sentence.id) ? next.delete(sentence.id) : next.add(sentence.id); return next; })}/><div><strong>{sentence.sourceText}</strong><div className="languageBadges">{sentence.languages.map(item => <span className={`languageBadge ${item.status}`} key={item.targetLanguage}>{item.targetLanguage}</span>)}{sentence.topics.map(item => <span className="topicBadge" key={item}>#{item}</span>)}</div>{sentence.error && <small className="errorText">{sentence.error}</small>}</div><span className={`badge ${sentence.status}`}>{labels[sentence.status]}</span><div className="sentenceActions"><button onClick={() => setEditingId(sentence.id)}>Карточка и перевод</button><button onClick={() => prepare([sentence.id])}>{sentence.languages.some(item => item.targetLanguage === targetLanguage && item.status === "ready") ? "Создать заново" : `Подготовить: ${targetLanguage}`}</button></div></article>) : <div className="empty">По выбранному фильтру предложений пока нет.</div>}</div>
    {editingId !== null && (
      <SentenceEditor sentenceId={editingId} onClose={() => setEditingId(null)} onSaved={load}/>
    )}
  </>;
}
