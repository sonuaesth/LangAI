...
const LANGUAGES = ["English", "German", "French", "Spanish", "Italian", "Portuguese", "Serbian", "Russian", "Ukrainian", "Polish", "Dutch", "Japanese", "Korean", "Chinese"];
...
export function SettingsView() {
  ...
  async function save() {
    try {
      await api.saveSettings(model, language);
  ...
  return <>
    ...
    <div className="settingsGrid">
      <section className="card"><h2>OpenAI API</h2><div className="keyState">{configured ? "● Ключ настроен" : "○ Ключ не настроен"}</div>
        ...
        <div className="actions"><button className="primary" disabled={!key} onClick={async () => { try { await api.verifyKey(key); await api.saveKey(key); setKey(""); setConfigured(true); await refreshModels(); setMessage("Ключ проверен, модели обновлены"); } catch (reason) { setMessage(String(reason)); } }}>Проверить и сохранить</button><button className="danger" disabled={!configured} onClick={async () => { await api.deleteKey(); setConfigured(false); setModels([]); setModel(""); }}>Удалить ключ</button></div>
      </section>
      <section className="card"><h2>Перевод</h2>
        ...
        <label className="field"><span>Целевой язык</span><select value={language} onChange={event => setLanguage(event.target.value)}>{LANGUAGES.map(item => <option value={item} key={item}>{item}</option>)}</select></label>
        <button className="primary" disabled={!model} onClick={save}>Сохранить</button>
      </section>
    </div>
